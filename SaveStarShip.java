...
public class SaveStarShip {
    ...
    public boolean isHangarOk(int side1, int side2, int price) {
        int width = Math.min(side1, side2);
        int length = Math.max(side1, side2);

        int area = width * length;

        boolean enoughArea = area >= 1500;
        boolean properShape = length <= 2 * width;
        boolean affordable = (price * 1.0 / area) <= 1000;

        return  enoughArea && properShape && affordable;
    }
